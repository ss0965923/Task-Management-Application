...
function renderCompletedTasks() {
  ...
  const validTasks = finishedTasks.filter(task => task && typeof task === "object");
  ...
  validTasks.forEach((task, index) => {
    ...
    taskItem.innerHTML = `
      ...
      <p>Deadline: ${task.deadline}</p>
      <p>Description: ${task.description}</p>
      <button type="button" class="delete" data-index="${index}">
        <span>Delete</span>
      </button>
    `; 
  ...
  completedTasksList.addEventListener("click", (event) => {
    if (event.target.classList.contains("delete")) {
      ...
      // Remove the task from the finishedTasks array
...
function addCompletedTask(task) {
  if (task !== null && typeof task === "object") {
    ...
    renderCompletedTasks(); // Call renderCompletedTasks after adding a task
  } else {
